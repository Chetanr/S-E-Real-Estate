...
{   
	...
	public void assignEmployee(Employee employee, Property property) throws PropertyAlreadyAssignedEmployee, IncorrectEmployeeAssigned
	{
		if((employee instanceof SalesConsultant) && (property instanceof Sale))
		{
			...
			{
				throw new PropertyAlreadyAssignedEmployee("A property can be assigned to only one employee. The property "+property.getProperty_id()+" is assigned to sales consultant "+saleProperty.getSalesConsultant().getID());
			}
			else
			{
				saleProperty.setSalesConsultant(salesConsultant);
				salesConsultant.setProperty(saleProperty);
				System.out.println("Successfully assigned property with id "+property.getProperty_id()+" to "+salesConsultant.getID());
			}
		}
		else if((employee instanceof PropertyManager) && (property instanceof Rental))
		{
			...
			{
				throw new PropertyAlreadyAssignedEmployee("A property can be assigned to only one employee.The property "+property.getProperty_id()+" is assigned to property manager "+rentalProperty.getPropertyManager().getID());
			}
			else
			{
				rentalProperty.setPropertyManager(propertyManager);
				propertyManager.setProperty(rentalProperty);
				System.out.println("Successfully assigned property with id "+property.getProperty_id()+" to "+propertyManager.getID());
			}
		}
		else 
		{
